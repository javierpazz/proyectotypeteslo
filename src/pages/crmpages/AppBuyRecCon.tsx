...
import { IComprobante, IConfiguracion, ICustomer,  IReceipt, ISupplier } from '../../interfaces';
        ...
        <Box mt={4}>
          <Paper variant="outlined" sx={{ p: 2 }}>
            <Box bgcolor="black" color="white" p={1} textAlign="center">
              ...
              <Typography variant="h6">ORDEN DE PAGO</Typography>
            </Box>

            <Grid container spacing={2} mt={2}>
              <Grid item xs={12} md={6}>
                <Typography><strong>{(receipt.id_config as IConfiguracion)?.name}</strong></Typography>
                <Typography><strong>RECIBO</strong> {(receipt.id_config as IConfiguracion)?.name}</Typography>
              ...
              <Grid item xs={12} md={6}>
                <Typography><strong>ORDEN DE PAGO</strong></Typography>
                <Typography><strong>Punto de Venta:</strong> {(receipt.id_config as IConfiguracion)?.codCon}</Typography>
                <Typography><strong>Comp. Nro:</strong> {receipt.recNum}</Typography>
                <Typography><strong>Fecha de Emision:</strong> {receipt.recDat?.substring(0, 10)}</Typography>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} md={6}>
                <Typography><strong>CUIT:</strong> {(receipt.supplier as ISupplier)?.cuit}</Typography>
                <Typography><strong>Condición IVA:</strong> {(receipt.supplier as ISupplier)?.coniva}</Typography>
              </Grid>
              <Grid item xs={12} md={6}>
                <Typography><strong>Apellido y Nombre / Razon Social:</strong> {(receipt.supplier as ISupplier)?.name}</Typography>
                <Typography><strong>Dirección:</strong> {(receipt.supplier as ISupplier)?.domcomer}</Typography>
              </Grid>
            </Grid>
