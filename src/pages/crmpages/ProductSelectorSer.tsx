...
export const ProductSelectorSer: FC<ProductSelectorProps> = ({ onSelectPro, productss }) => {
  ...
  return (
    <div style={{ padding: '10px' }}>
      <label htmlFor="codeInput">Código de Tarea:</label>
      <input
        id="codeInput"
        ref={inputRef}
        type="text"
      ...
      {filtered.length > 0 && (
        ...
        >
          {filtered.map((p, index) => (
            ...
            >
              <strong>{p.codigoPro}</strong> - {p.title} - Valor - ${p.price}
            </li>
          ))}
        </ul>
      )}
