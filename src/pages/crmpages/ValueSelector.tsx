...
interface ValueSelectorProps {
  onSelectVal: (value: IValue) => void;
  valuess: IValue[];
}


// export const ProductSelector = ( {onSelectVal,valuess} ) => {
export const ValueSelector: FC<ValueSelectorProps> = ({ onSelectVal, valuess }) => {
  const [search, setSearch] = useState('');
  const [filtered, setFiltered] = useState(valuess);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      onSelectVal(filtered[highlightedIndex]);
    }
  };
  ...
  return (
    <div style={{ padding: '10px' }}>
      <label htmlFor="codeInput">Código de Valor:</label>
      <input
        id="codeInput"
        ref={inputRef}
        type="text"
