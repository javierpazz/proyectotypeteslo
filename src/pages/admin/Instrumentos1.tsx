...
export const Instrumentos = () => {
   ...
   const [rows, setRows] = useState<RowsAux[]>([]);
    ...
    const loadData = async() => {
        try {
          ...
          setInstrumentos(resp.data);
            const rowsAux = resp.data.map( instru => ({
                id: instru._id,
                codIns: instru.codIns,
                name: instru.name,
            }));
            setRows(rowsAux as any);
            console.log("rowsAux")
            console.log(rowsAux)
    ...
    //   setInstrumentos((prev) => prev.filter((ins) => ins._id !== id));
