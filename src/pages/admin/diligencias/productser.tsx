...
export const ProductSerAdminPage = () => {
    ...
    useEffect(() => {
        if (!user && !isLoading) {
        navigate('/auth/loginadm?redirect=/admin/productsser');
        }
        if (user?.role === "client" ) {
        navigate('/');
        }
...
const loadProduct = async() => {
    ...
    try {
        ...
        productI.medPro=resp.data.medPro,
        productI.images=resp.data.images,
        productI.inStock=resp.data.inStock,
        productI.price=resp.data.price,
        productI.priceBuy=resp.data.priceBuy,
        productI.porIva=resp.data.porIva,
